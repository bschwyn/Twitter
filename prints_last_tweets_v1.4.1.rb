...
tweetcount= 20 #number of tweets from each username to print
$PUNCTUATION = /[,!.#~`$%^&*()-=+\/\[{\]}\|;:'\",<.>?\\+ \t]+/
...
def tweets_to_array(username,access_token,tweetcount)
	...
	# Does not currently work, but would like an error message in case username does not exist.
	#if hash["code"] ==34
	#	puts hash["message"]
	#end
...
def print_all(array_of_usernames, access_token,tweetcount)
	if tweetcount > 200
	...
	array_of_usernames.each { |name|
		tweets_array = tweets_to_array(name, access_token, tweetcount)
	...
	graph = create_big_graph(array_of_usernames, access_token, tweetcount)
...
def create_big_graph(array_of_names, access_token, tweetcount)
...
print_all(array_of_names, access_token, tweetcount)
