...
name_file = "famous_people.txt" #name of file to read usernames from
...
def tweets_to_array(username,access_token,tweetcount,file)
	...
	array = []
	result.map{ |hash| hash["text"] }
end
	
def print_all_tweets2(array_of_usernames, access_token,tweetcount)
	if tweetcount > 200
	...
	array_of_usernames.each { |name|
		...
		puts tweets_to_array(name,access_token, tweetcount, file)
		file.puts tweets_to_array(name,access_token, tweetcount, file)
	}
	file.close
end
#*****************************************************************************************
...
print_all_tweets2(array_of_names,access_token,tweetcount)
