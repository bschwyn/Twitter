...
def tweet_file_parser(file)
	...
	string_array.map{ |k|k[1..-1].downcase } #needs to be -1 if array only has 1, otherwise -2
	#error in split means the number is wrong
end
...
def tweets_to_array(username,token,tweetcount) #rename "getsTweetText"
	...
	tweets_array = result.map{ |hash|
		...
		if hash.size ==2
			...
			if hash[1][0]["code"] !=34
				return "program ended prematurely\nError: #{hash[1][0]["message"]}"
			end
		else
...
def gets_username(array)   #note, possible error if somebody has "@joe@" in their tweet
	mentions = []
	array.each {|line|
